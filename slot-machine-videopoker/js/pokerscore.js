...
function getPokerScoreText(val) {
	...
	switch (true) {
		case (pairType == 6):
			text = "Five<br />" + pairHigh + "s";
			break;
		case (hasStraightFlush == 1):
			text = (((val4[0] != null) && ((val4[0]).isTen) && (val4[4] != null) && ((val4[4]).isAce)) ? "Royal" : "Straight");
			text += "<br />Flush<br />";
		...
		case (hasFlush == 1):
			...
			break;
		case (hasStraight == 1):
			text = "Straight<br />" + ((val4[0] != null) ? ((val4[0]).scoretext).replace('"', "") : "?");
			text += " - " + ((val4[4] != null) ? ((val4[4]).scoretext).replace('"', "") : "?");
			break;
		case (pairType == 3):
			text = "Three<br />" + pairHigh + "s";
			break;
		case (pairType == 2):
			text = "Two Pair<br />" + pairHigh + "s & " + pairLow + "s";
			break;
		case (pairType == 1):
			switch (true) {
				case (jQuery.inArray(pairHigh, acceptablePairs) !== -1):
					text = "Two<br />" + pairHigh + "s";
					break;
				default:
					text = "Nothing";
					break;
...
function getPokerScoreValue(val) {
	...
	switch (true) {
		case (pairType == 6):
			...
			break;
		case (hasStraightFlush == 1):
			value = (((val4[0] != null) && ((val4[0]).isTen) && (val4[4] != null) && ((val4[4]).isAce)) ? 40 : 30);
			break;
		case (pairType == 5):
			value = 20;
			break;
